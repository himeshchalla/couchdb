...
validate_docid(DocId, DbName) ->
    Whitelist = config:get("couchdb", "whitelist_doc_ids",
        [{"_dbs", ["_users", "_replicator", "_global_changes"]}]),
    DocIds = proplists:get_value(couch_util:to_list(DbName), Whitelist, []),
    case lists:member(couch_util:to_list(DocId), DocIds) of
        true ->
            ok;
        false ->
